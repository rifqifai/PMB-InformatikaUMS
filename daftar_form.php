...
$p_asal_skl		= isset($_POST['asal_skl']) ? $_POST['asal_skl'] : "";
$p_sc_alamat	= isset($_POST['sc_alamat']) ? $_POST['sc_alamat'] : "";
...
if ($mode_form == "add") {
	...
	$p_asal_skl	= $a_data_edit[18];
	$p_sc_alamat= $a_data_edit[20];
...
if ($p_tombol == "DAFTAR") {
	...
	if ($p_nama == "" || $p_jk == "" || $p_agama == "" || $p_tmp_lahir == "" 
		...
		|| $p_nama_ay == ""	|| $p_nama_ib == "" || $p_pend_ay == "" || $p_pend_ib == "" || $p_pkj_ay == ""
		|| $p_pkj_ib == "" || $p_asal_skl == "" || $p_sc_alamat == "" || $p_thn_lulus == "" || $p_no_ijazah == "" || $p_bing == ""
		|| $p_bind == "" || $p_mtk == "" || $p_ipa == "") {
		...
		echo "<h4 class='alert_error'>Jurusan Pilihan 1 dan 2 masih KOSONG<span id='close'>[<a href='#'>X</a>]</span></h4>";
			...
			$q_cek_sek_asal = mysql_query("SELECT * FROM t_skolah WHERE skolah = '$p_asal_skl'");
			$j_cek_sek_asal = mysql_num_rows($q_cek_sek_asal);
			
			//INSERT DATA Sekolah Asal Otomatis jika belum ada */
			if ($j_cek_sek_asal < 1) {
				$q_sek_baru = mysql_query("INSERT INTO t_skolah VALUES ('', '$p_asal_skl', '$p_sc_alamat')");
			...
			if ($j_cek_ganda > 0) {
				echo "<h4 class='alert_error'>Data Ganda Disebabkan Auto Refresh<span id='close'>[<a href='#'>X</a>]</span></h4>";
			} else {
	...
	if ($p_nama == "" || $p_jk == "" || $p_agama == "" || $p_tmp_lahir == "" 
		...
		/* PENGECEKAN Simpan Sekolah Asal jika BELUM ADA */
		$q_cek_sek_asal = mysql_query("SELECT * FROM t_skolah WHERE skolah = '$p_asal_skl' AND alamat = '$p_sc_alamat'");
		$j_cek_sek_asal = mysql_num_rows($q_cek_sek_asal);
		...
		$q_daftar	= mysql_query("UPDATE master SET 
				 ...
				 s_alamat = '$p_alamat', s_hp = '$p_hp', k_nama_ay = '$p_nama_ay',
				 ...
				 sc_alamat = '$p_sc_alamat', nil_1_bing = '$p_bing', 
...
<article class="module width_full">
	<header><h3>Data Calon Mahasiswa Baru | <a href="?p=daftar_form&mod=add"> Tambah Data</a></h3></header>
	...
	<table class="tbl_form">
		...
		cInput("Nomor Handphone", "s_hp", "20", $p_s_hp);
		?>
	</table>
	</div>
</article>
